...
public:
    ...
    void channel(int ch, int value) {
        ...
        std::lock_guard<std::mutex> lock(dmx_mutex);
        dmx_data[ch] = static_cast<unsigned char>(value);
    }
    ...
    void send() {
        ...
        // Snapshot all state we need under state_mutex
        ...
        case Protocol::Serial_Raw:
        case Protocol::Serial:
            send_Serial(snapshot, current_protocol);
            break;
        case Protocol::sACN:
            send_sACN(snapshot, current_universe);
            break;
        case Protocol::ArtNet:
    ...
    bool priority(int p) {
        ...
        std::lock_guard<std::mutex> lock(state_mutex);
        _sacn_priority = p;
        // If sACN is already running, update priority live
        if (_sacn_initialized) {
            etcpal::Error err = source.ChangePriority(static_cast<uint16_t>(_universe), static_cast<uint8_t>(p));
            if (!err.IsOk()) {
                std::cerr << "DMX Warning: Failed to change sACN priority: " << err.ToString() << std::endl;
                return false;
            }
        }
        return true;
    ...
    void fade(int ch, int target, int durationMs) {
        ...
        std::lock_guard<std::mutex> flock(fade_mutex);
        FadeState& f = _fades[ch];
        {
            std::lock_guard<std::mutex> dlock(dmx_mutex);
            f.start_value = dmx_data[ch];
        }
...
private:
    ...
    std::mutex state_mutex;   // protects protocol, universe, rate, last_send_time, init state
    std::mutex fade_mutex;    // protects _fades
    ...
    FadeState _fades[513]; // index 0 unused
    ...
    bool init_sACN() {
        try {
            ...
            if (!error.IsOk()) {
                std::cerr << "DMX Error: sACN AddUniverse failed: " << error.ToString() << std::endl;
                source.Shutdown();
                sacn_global_deinit();
                return false;
            }
    ...
    void send_sACN(const unsigned char* snapshot, int uni) {
        try {
            source.UpdateLevels(static_cast<uint16_t>(uni), snapshot + 1, 512);
        }
        catch (const std::exception& e) {
...
CK_DLL_MFUN(dmx_port) {
    ...
    if (!dmx_obj) { RETURN->v_string = API->object->create_string(VM, name.c_str(), (t_CKUINT)name.length()); return; }

    dmx_obj->port(name);
    RETURN->v_string = API->object->create_string(VM, name.c_str(), (t_CKUINT)name.length());
}
...
CK_DLL_QUERY(DMX) {
    ...
    QUERY->doc_func(QUERY,
        "Get the current value (0-255) of a DMX channel (1-512). Returns 0 if out of range."
    );
    ...
    QUERY->doc_func(QUERY,
        "Set a DMX channel (1-512) to a value (0-255). Values outside 0-255 are clamped. "
    ...
    QUERY->doc_func(QUERY,
        "Set multiple consecutive DMX channels starting at startChannel. "
    ...
    QUERY->doc_func(QUERY,
        "Initialize the DMX connection using the configured protocol, port, universe, and priority. "
    ...
    QUERY->doc_func(QUERY,
        "Transmit the current DMX buffer over the active protocol. "
    ...
    QUERY->doc_func(QUERY,
        "Set all 512 DMX channels to 0 and cancel all active fades. "
    ...
    QUERY->doc_func(QUERY,
        "Set the DMX universe for sACN or ArtNet output (1-63999). "
        "Configure this before init()."
    );
    ...
    QUERY->doc_func(QUERY,
        ...
        "Can be changed before or after init(); if sACN is already running, the priority updates live."
    );
